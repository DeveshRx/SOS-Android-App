...
public class ENote extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editText.setText(NoteActivity.notesArray.get(NoteActivity.itemNumber));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_discardNote2) {
            ...
            NoteActivity.notesArray.remove(NoteActivity.itemNumber);
            NoteActivity.displayArray.remove(NoteActivity.itemNumber);

            NoteActivity.theListView.setAdapter(NoteActivity.theAdapter);

            try {
                FileOutputStream fileOutputStream = openFileOutput("noteSaves", Context.MODE_PRIVATE);
                ObjectOutputStream objectOutputStream = new ObjectOutputStream(fileOutputStream);

                objectOutputStream.writeObject(NoteActivity.notesArray);
        ...
        if (id == R.id.action_saveNote2) {
            ...
            NoteActivity.notesArray.set(NoteActivity.itemNumber, theText);

            //set updated note up to newline to display array
            String[] beforeNewline = theText.split("\n", 20);
            NoteActivity.displayArray.set(NoteActivity.itemNumber,beforeNewline[0]);
